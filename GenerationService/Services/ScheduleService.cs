    ...
    public class ScheduleService : IScheduleService
    {
        ...
        public async Task<List<Schedule>> GetScheduleByClass(int classId)
        {
            ...
            if (generated.Any())
            {
                ...
                _db.SaveChanges();
                return generated.Where(s => s.ClassId == classId).OrderBy(s => s.DayOfWeek).ThenBy(s => s.LessonNumber).ToList();
            }
        ...
        public List<Schedule> GetScheduleByTeacher(int teacherId)
        {
            ...
            if (generated.Any())
            {
                _db.Schedules.AddRange(generated);
                _db.SaveChanges();
                return generated.Where(s => s.TeacherId == teacherId).OrderBy(s => s.DayOfWeek).ThenBy(s => s.LessonNumber).ToList();
            }
            return new List<Schedule>();
        }
        ...
        public byte[] GetSchedulePdfByClass(int classId)
        {
            ...
            return GeneratePdf(schedule.Result, $"Расписание для класса {classId}");
        }

        public byte[] GetSchedulePdfByTeacher(int teacherId)
        {
            var schedule = GetScheduleByTeacher(teacherId);
            return GeneratePdf(schedule, $"Расписание для учителя {teacherId}");
        }
        ...
        private byte[] GeneratePdf(List<Schedule> schedule, string title)
        {
            ...
            double y = 40;
            gfx.DrawString(title, boldFont, PdfSharpCore.Drawing.XBrushes.Black, new PdfSharpCore.Drawing.XRect(0, y, page.Width, 20), PdfSharpCore.Drawing.XStringFormats.TopCenter);
            y += 30;

            // Заголовок таблицы
            string[] headers = { "День", "№", "Время", "Класс", "Предмет", "Учитель" };
            double[] colWidths = { 80, 30, 80, 60, 150, 150 };
            double x = 40;
            for (int i = 0; i < headers.Length; i++)
            {
                gfx.DrawString(headers[i], boldFont, PdfSharpCore.Drawing.XBrushes.Black, new PdfSharpCore.Drawing.XRect(x, y, colWidths[i], 20), PdfSharpCore.Drawing.XStringFormats.TopLeft);
                x += colWidths[i];
            }
            y += 22;
            ...
            foreach (var item in sorted)
            {
                ...
                x = 40;
                string subjName = subjectDict.TryGetValue(item.SubjectId, out var sname) ? sname : item.SubjectId.ToString();
                string teacherName = teacherDict.TryGetValue(item.TeacherId, out var tname) ? tname : item.TeacherId.ToString();
                string className = classDict.TryGetValue(item.ClassId, out var cname) ? cname : item.ClassId.ToString();
                string[] row = {
                ...
                {
                    gfx.DrawString(row[i], font, PdfSharpCore.Drawing.XBrushes.Black, new PdfSharpCore.Drawing.XRect(x, y, colWidths[i], 20), PdfSharpCore.Drawing.XStringFormats.TopLeft);
                    x += colWidths[i];
                }
                y += 20;
                if (y > page.Height - 40)
        ...
        private List<Schedule> GenerateScheduleWithVersion(ScheduleVersion? version = null)
        {
            ...
            foreach (var cl in classes)
            {
                foreach (var lesson in classLessons.Where(l => l.ClassId == cl.Id))
                {
                    ...
                    var teacher = teacherSubjects.FirstOrDefault(ts => ts.SubjectId == lesson.SubjectId);
                    if (teacher != null)
                    {
            ...
            foreach (var cl in classes)
            {
                ...
                int totalLessons = lessons.Sum(l => l.HoursPerWeek);
                int maxWeeklyLessons = Math.Min(35, totalLessons);
                int placedLessons = 0;
                ...
                foreach (var day in daysOfWeek)
                {
                    ...
                    while (lessonsInDay < maxLessonsPerDay && subjectHoursLeft.Values.Any(h => h > 0))
                    {
                        ...
                        int teacherId = teacherByClassSubject.TryGetValue((cl.Id, subjectId), out var tid) ? tid : 0;
                        if (teacherId == 0) { subjectHoursLeft[subjectId]--; continue; }
                        int lessonNumber = lessonsInDay + 1;
                        if (classScheduleMap.ContainsKey((cl.Id, day, lessonNumber))) { lessonsInDay++; continue; }
                        if (teacherScheduleMap.ContainsKey((teacherId, day, lessonNumber))) { lessonsInDay++; continue; }
